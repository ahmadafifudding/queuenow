...
- [ ] 1. Scaffold Expo foundation, monorepo wiring, and env validation
  - [ ] 1.1 Initialize the `apps/mobile` Expo + TypeScript-strict project
  ...
  - [ ] 1.2 Configure Metro for the monorepo and declare workspace dependencies
  ...
  - [ ] 1.3 Implement the env validator and `app.config.ts`
...
- [ ] 2. Build the REST client layer and shared query infrastructure
  - [ ] 2.1 Implement the REST client (`src/lib/api/client.ts`)
  ...
  - [ ] 2.2 Create the QueryClient, query keys, and invalidation helpers
  ...
  - [ ] 2.3 Add the i18n string catalog and error-code message map
  ...
  - [ ] 2.4 Build the boundary test harness
  ...
  - [ ]\* 2.5 Write property test for envelope unwrapping and failure normalization
  ...
  - [ ]\* 2.6 Write property test for the authenticated-request auth header
  ...
  - [ ]\* 2.7 Write property test for single-flight 401 refresh-and-retry
  ...
  - [ ]\* 2.8 Write property test for the error-code-to-message map
...
- [ ] 3. Build the realtime socket client and resilience layer
  - [ ] 3.1 Implement the Realtime Client and subscription bridge (`src/lib/socket.ts`)
  ...
  - [ ] 3.2 Implement bounded connect-and-resubscribe retry and reconnect triggers
  ...
  - [ ]\* 3.3 Write property test for the ticket-update bridge key targeting
  ...
  - [ ]\* 3.4 Write property test for subscription lifecycle
  ...
  - [ ]\* 3.5 Write property test for reconnect re-issuing tracked subscriptions
  ...
  - [ ]\* 3.6 Write property test for the retry backoff schedule
...
- [ ] 4. Build the Auth Manager, secure token storage, and device fingerprint
  - [ ] 4.1 Implement secure-store, auth-store, and device fingerprint
  ...
  - [ ] 4.2 Implement the Auth Manager (`src/lib/auth/auth-manager.ts`)
  ...
  - [ ]\* 4.3 Write property test for the secure-token write-confirmation invariant
  ...
  - [ ]\* 4.4 Write property test for account-input schema validation
  ...
  - [ ]\* 4.5 Write property test for account data isolation when signed out
...
- [ ] 5. Build timezone formatting and offline cache/connectivity utilities
  - [ ] 5.1 Implement timezone-aware formatting (`src/lib/format.ts`)
  ...
  - [ ] 5.2 Implement the offline cache and connectivity bridge
  ...
  - [ ]\* 5.3 Write property test for offline cache round-trip, staleness, and action gating
...
- [ ] 6. Build the Notification Manager and push registration
  - [ ] 6.1 Implement the Notification Manager (`src/lib/notifications/manager.ts`)
  ...
  - [ ] 6.2 Implement push-token registration and deep-link on activation (`src/lib/notifications/push.ts`)
  ...
  - [ ]\* 6.3 Write property test for turn-notification mapping
  ...
  - [ ]\* 6.4 Write property test for notification channel selection
  ...
  - [ ]\* 6.5 Write example test for push payload deep-linking
    - Delivered push payload for an Active_Ticket resolves to the `/ticket/[orgId]/[ticketId]` route on activation
    - _Requirements: 13.3_

- [ ] 7. Implement organization and service discovery (MVP)
  - [ ] 7.1 Implement QR/deep-link parsing, manual code entry, and service resolution
  ...
  - [ ]\* 7.2 Write property test for QR/deep-link parse round-trip
    - **Property 1: QR / deep-link parse round-trips**
    - **Validates: Requirements 1.1**
    - Location: `src/features/discovery/__tests__/qr-parse.property.test.ts`

  - [ ]\* 7.3 Write property test for the discovery join-availability decision
  ...
  - [ ]\* 7.4 Write example test for the manual-code discovery happy path
    - Manual code resolves the org and renders its active services
    - _Requirements: 1.2_

- [ ] 8. Implement join queue and ticket result (MVP)
  - [ ] 8.1 Implement join request construction and the join/result screen
  ...
  - [ ]\* 8.2 Write property test for join request construction and validation
  ...
  - [ ]\* 8.3 Write property test for verbatim position and wait
...
- [ ] 9. Implement active-ticket tracking, realtime, offline, and turn alerts (MVP)
  - [ ] 9.1 Implement the active-ticket tracking screen
...
- [ ] 11. Implement the optional account auth screens (Phase 2)
  ...
  - [ ]\* 11.2 Write example tests for the auth flow
    - Register/login success and failure, sign-out clears state, submit disabled while pending
    - _Requirements: 6.1, 6.2, 6.3, 6.6_

- [ ] 12. Implement ticket history (Phase 2)
  - [ ] 12.1 Implement the history screen
  ...
  - [ ]\* 12.2 Write property test for reverse-chronological ordering
  ...
  - [ ]\* 12.3 Write property test for required list-row fields
  ...
  - [ ]\* 12.4 Write integration test for the history hook
    - The history query calls `GET /customers/history`
    - _Requirements: 7.1_

- [ ] 13. Implement favorite organizations (Phase 2)
  - [ ] 13.1 Implement the favorites screen and mutations
  ...
  - [ ]\* 13.2 Write integration test for favorites list/add/remove
    - The hooks call the correct list/add/remove paths and methods
    - _Requirements: 8.1, 8.2, 8.3_

- [ ] 14. Implement the notifications list and push registration wiring (Phase 2)
  - [ ] 14.1 Implement the notifications list screen
  ...
  - [ ]\* 14.2 Write integration test for notifications list and push-token registration
...
- [ ] 15. Checkpoint - Phase 2 (account features) complete
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 16. R11 — Customer leave/cancel ticket (Backend Dependency)
  - [ ] 16.1 [apps/api] Add the public, ownership-scoped cancel endpoint to the queue module
  ...
  - [ ]\* 16.2 [apps/api] Write tests for the cancel endpoint
    - Ownership acceptance/rejection, non-`WAITING` rejection with the mapped code, and event emission
    - _Requirements: 11.2, 11.4_

  - [ ] 16.3 [apps/mobile] Wire the Leave/cancel flow with graceful degradation
...
- [ ] 17. R12 — Customer token refresh (Backend Dependency)
  - [ ] 17.1 [apps/api] Add the customer refresh endpoint to the customer module
  ...
  - [ ]\* 17.2 [apps/api] Write tests for the refresh endpoint
    - Valid refresh rotates the session and returns a new pair; invalid/expired returns the auth error code
    - _Requirements: 12.2, 12.4_

  - [ ] 17.3 [apps/mobile] Finalize the refresh-and-retry wiring with degradation
...
- [ ] 18. R13 — Push notification delivery (Backend Dependency)
  - [ ] 18.1 [apps/api] Implement push delivery in `NotificationService.sendNotification`
  ...
  - [ ]\* 18.2 [apps/api] Write tests for push delivery
  ...
  - [ ] 18.3 [apps/mobile] Wire the background-delivered push handler and deep link
...
- [ ] 19. Checkpoint - Phase 3 (backend dependencies) complete
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 20. Offline hardening and cross-cutting quality gates (Phase 4)
  - [ ] 20.1 Harden app-wide offline behavior and connectivity restore
  ...
  - [ ] 20.2 Establish the accessibility baseline
  ...
  - [ ]\* 20.3 Write integration tests for connectivity restore and manual refresh
  ...
  - [ ]\* 20.4 Add build/smoke and strict-typecheck quality gates
